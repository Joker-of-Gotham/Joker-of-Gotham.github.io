...
`summary` 是列表和搜索简介的唯一来源；省略或留空时不显示简介，不从正文提取。博客、研究、书评、杂谈和搜索共用行内预览：保留文字、加粗、斜体、删除线、行内代码与 `$…$` 行内公式，链接只保留文字；标题、段落、列表和引用中的文字合并成连续简介。表格、图片及图片引用、HTML、代码块、`$$…$$` 行间公式整块跳过，不占简介长度。先解析再缩短，完整行内代码和公式不会被切开；已有截断留下的 Markdown 符号会清理，不完整图片、标签及公式片段会跳过，不猜测补写。多行题头使用 YAML 的 `summary: |`；完整块级内容请放在正文中。
