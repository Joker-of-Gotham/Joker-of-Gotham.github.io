...
tags:
  - 综述笔记
  - 金融量化
slug: 2025-08-07-machine-learning-for-trade-algo-p2
collection: llm_learning
summary: "金融特征工程——α因子研究\r \r 算法交易策略由信号驱动，这些信号指示何时买卖资产，以期获得相对于基准（如指数）的超额回报。资产回报中⽆法由该基准⻛险敞⼝解释的部分被称为阿尔法（alpha），因此，旨在产⽣此类⾮相关回报的信号也被称为阿尔法因⼦。数⼗年的研究积累了⼤量关于市场运作"
related_nodes: []
---
# 金融特征工程——α因子研究
...
5. 凯利准则及其应用
    ...
    - 最优下注规模
        ...
        $$
        \begin{aligned}
        V_{N} &= (1+b \times f)^{w}(1-f)^{L}V_{0} \\
        G &= \lim\limits_{N \to \infty}\left[\frac{W}{N}\log(1+b \times f)+\frac{L}{N}\log(1-f)\right] \\
          &= p\log(1+b \times f)+(1-p)\log(1-f)
        \end{aligned}
        $$
